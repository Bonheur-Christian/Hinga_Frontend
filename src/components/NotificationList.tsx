...
const NotificationList = () => {
  ...
  const fetchNotifications = async () => {
    try {
      ...
      const response = await fetch("http://localhost:4002/api/notifications/all", {
        headers: {
  ...
  const markAsRead = async (notificationId: number) => {
    try {
      ...
      const response = await fetch(`http://localhost:4002/api/notifications/${notificationId}/read`, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
