...
const BuyerDashboard = () => {
  ...
  const [selectedProduct, setSelectedProduct] = useState<Livestock | null>(
    null
  );
  ...
  const fetchLivestock = async () => {
    try {
      ...
      const response = await fetch(
        "http://localhost:4002/api/livestock/v2/all",
        {
          headers: {
      ...
      const initialQuantities = data.reduce(
        (acc: { [key: number]: number }, item: Livestock) => {
  ...
  const addToCart = async (livestock: Livestock) => {
    try {
      ...
      const response = await fetch(
        "http://localhost:4002/api/cart/create-cart",
        {
          method: "POST",
          headers: {
  ...
  const removeFromCart = async (cartId: number) => {
    try {
      ...
      const response = await fetch(
        `http://localhost:4002/api/cart/${cartId}/${decoded.id}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const updateCartQuantity = async (cartId: number, newQuantity: number) => {
    try {
      ...
      const response = await fetch(`http://localhost:4002/api/cart/${cartId}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
  ...
  const currentProducts = filteredAndSortedLivestock.slice(
    indexOfFirstProduct,
    indexOfLastProduct
  );
  const totalPages = Math.ceil(
    filteredAndSortedLivestock.length / productsPerPage
  );
            ...
            {currentProducts.map((item) => (
              ...
              >
                <CardHeader>
                  ...
                  <CardDescription>Breed: {item.breed}</CardDescription>
                </CardHeader>
                <CardContent>
                    ...
                    <img
                      src={
                        item.imageUrls.startsWith("http")
                          ? item.imageUrls
                          : `http://localhost:4002${item.imageUrls}`
                      }
                      alt={item.type}
                  ...
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        handleQuantityChange(item.livestockId, e.target.value)
                      }
                    ...
                    <Button
                      ...
                      size="sm"
                      onClick={() =>
                        incrementQuantity(item.livestockId, item.quantity)
                      }
                  ...
                  <div className="text-sm text-gray-500 mb-4">
                    ...
                    {Math.max(
                      0,
                      item.quantity - (quantities[item.livestockId] || 1)
                    )}
                  </div>
                </CardContent>
                <CardFooter className="flex flex-col gap-2">
                  ...
                  <Button
                    ...
                    disabled={
                      item.quantity === 0 ||
                      quantities[item.livestockId] > item.quantity
                    }
                  >
                    Add to Cart
                  </Button>
            ...
            <>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="relative aspect-video">
                  <img
                    src={
                      selectedProduct.imageUrls.startsWith("http")
                        ? selectedProduct.imageUrls
                        : `http://localhost:4002${selectedProduct.imageUrls}`
                    }
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-center gap-2">
                    <Button
                      ...
                      size="sm"
                      onClick={() =>
                        decrementQuantity(selectedProduct.livestockId)
                      }
                    ...
                    <Button
                      ...
                      disabled={
                        quantities[selectedProduct.livestockId] >=
                        selectedProduct.quantity
                      }
                    >
                      +
