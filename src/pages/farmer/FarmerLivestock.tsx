...
const FarmerLivestock = () => {
  ...
  const fetchLivestock = async () => {
    try {
      ...
      const response = await fetch("http://localhost:4002/api/livestock/v2/get", {
        headers: {
  ...
  const onSubmit = async (data: LivestockFormValues) => {
    try {
      ...
      const url = editingLivestockId
        ? `http://localhost:4002/api/livestock/v2/${editingLivestockId}`
        : "http://localhost:4002/api/livestock/v2/add";
  ...
  const deleteLivestock = async (id: number) => {
    try {
      ...
      const response = await fetch(`http://localhost:4002/api/livestock/v2/${id}`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`,
        },
            ...
            <Card key={item.livestockId}>
              ...
              <CardContent>
                <div className="space-y-3">
                  ...
                  {item.imageUrls && (
                    <img
                      src={item.imageUrls.startsWith('http') ? item.imageUrls : `http://localhost:4002${item.imageUrls}`}
                      alt={item.type}
